        ...
        <div className="container">
            ...
            <div className={`tab-content ${classes.color}`} id="nav-tabContent" >
                ...
                <div className="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab" >
                    {cards.length ? (<div> {cards.map((card: Card) => (<p className="text-start mt-3" style={{ padding: "1rem" }} key={card._id}><span style={{ fontWeight: "bold" }}>{card.title}:</span> {card.email} </p>))} </div>) : (<p>No Clients To Show</p>)}
                </div>
            </div></div>
    </>);
}
